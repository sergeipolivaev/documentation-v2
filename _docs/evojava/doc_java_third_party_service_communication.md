...
{% include important.html content="Установленные на смарт-терминале приложения могут обращаться только к сторонним сервисам по протоколам HTTP и HTTPS. При этом, если сторонние сервисы находятся в одной локальной сети со смарт-терминалом, облако не [проксирует запросы](./doc_java_third_party_service_communication.html#cloudproxy). Обращаться к REST API Облака Эвотор нельзя." %}
