...
* События -- на intent'ы похожи очень эти события.
* Команды --
* Широковещательные сообщения --

## События

События во многом напоминают intent'ы:


Всего существует восемь событий, которые распространяет смарт-терминал:

* Событие, которое возникает при намерении изменить позицию чека: [BeforePositionsEditedEvent](./).
* Событие, которое возникает при разделении платежей в чеке продажи: [PaymentSelectedEvent](./).
* Событие, которое возникает при начислении скидки на чек: [ReceiptDiscountEvent](./).
* Событие, которое возникает при разделении чека на печатные группы: [PrintGroupRequiredEvent](./).
* Событие, которое возникает при объединении позиций чека: [PositionsMergeEvent](./).
* Родительский класс событий, которые возникают при оплате чека сторонней платёжной системой: [PaymentSystemEvent](./).

   * Производное событие, которое возникает при оплате чека продажи сторонней платёжной системой
   * Производное событие, которое возникает при отмене оплаты чека продажи сторонней платёжной системой
   * Производное событие, которое возникает при оплате чека возврата сторонней платёжной системой
   * Производное событие, которое возникает при отмене оплаты чека возврата сторонней платёжной системой

* Событие, которое возникает при передаче оплаты чека продажи другому приложению (например, при выборе приложения "Комбооплата"): [PaymentDelegatorEvent](./)
* Событие, которое возникает при печати в чеке произвольных данных: [PrintExtraRequiredEvent](./)
...
Unlike Events, Commands are isuued by an application and it is expected to make terminal do something in respond to the Command
